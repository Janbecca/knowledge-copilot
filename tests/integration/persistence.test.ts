...
describe("SQLite persistence", () => {
  ...
  it("migrates existing sessions to the safe host-structured default", () => {
    ...
    expect(store.db.prepare("SELECT version FROM schema_migrations ORDER BY version").all()).toHaveLength(5);
    expect(store.db.prepare("SELECT name FROM sqlite_master WHERE type='table' AND name='conversation_bindings'").get()).toBeTruthy();
    store.close();
  });
});
