...
export class KnowledgeService {
  ...
  launch(input:{session_id?:string;title?:string;source_host?:string}={}){const session=input.session_id?this.requireSession(input.session_id):(input.source_host?this.store.findActiveSessionBySourceHost(input.source_host):null)??this.start({title:input.title,source_host:input.source_host??"chatgpt"});return {...this.get(session.session_id),capture_policy:"continuous_until_paused",next_tool:"capture_active_learning_turn" as const};}
  ...
  async capture(input:{session_id?:string;user_message:string;assistant_message:string;tool_observations?:string[];source_reference?:string;idempotency_key?:string;source_host?:string;knowledge_items?:ModelKnowledgeItem[]}){
    ...
    const result=input.knowledge_items?{events:this.modelEvents(turn,input.knowledge_items,this.store.listCards(session.session_id))}:await this.extractor.extract({session,turn,activeCards:this.store.listCards(session.session_id)});
