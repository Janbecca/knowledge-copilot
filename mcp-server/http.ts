...
export async function startHttp(service: KnowledgeService, config: RuntimeConfig): Promise<Server> {
  ...
  const server = createServer(async (req, res) => {
    ...
    try {
      ...
      const extractionMode = url.pathname.match(/^\/api\/sessions\/([^/]+)\/extraction-mode$/); if (extractionMode && req.method === "POST") { json(req, res, config, 200, service.changeExtractionMode({ ...(await body(req, config.bodyLimitBytes) as object), session_id: extractionMode[1]! } as never)); return; }
