...
export function createMcpServer(service:KnowledgeService):McpServer{
  ...
  registerAppTool(server,"open_knowledge_panel",{title:"Open knowledge panel",description:"Read one learning session and render its interactive knowledge panel without modifying state.",inputSchema:{session_id:z.string()},outputSchema:sessionStateSchema,annotations:annotations.read,_meta:{ui:{resourceUri:PANEL_URI}}},input=>safely(()=>service.get(input.session_id)));
